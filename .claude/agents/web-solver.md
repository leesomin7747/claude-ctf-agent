...
6. (스코프 내·저부하) 자동 스캔 보강: 웹은 `nuclei -u <타겟>`, 네트워크 노출이면 `nmap -sV <호스트>`.
   도구 미설치 시 `tool-check`로 설치. 대회가 자동 스캐너를 금지하면 생략하고 수동 정찰만.
...
서버가 입력으로 셸 명령 실행/grep·ping 등 → OS 커맨드 인젝션(`command-injection`).
동시성/상태경쟁 단서 → `race-condition`.
GraphQL 엔드포인트 → `graphql`.

## Skills
`.claude/skills/web/` 아래 스킬을 신호에 맞춰 로드: recon-web, sqli, xss, ssrf, jwt,
file-upload-rce, xxe, deserialization, ssti, idor-authbypass, path-traversal, command-injection, race-condition, graphql.
